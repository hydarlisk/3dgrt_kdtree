...
namespace AABB_Triangle_Clipping {
	...
	inline bool _inside(float coord, float boundary, _side side) {//box in out 판단용
		bool result;
		if (side == _MIN)
		...
		for (int index_prev = 0; index_prev < pg_from_size; index_prev++) {//edge별로 확인
			...
			if (inside_prev) {
				if (inside_cur) { // prev in & cur in
					pg_to.push_back(pg_from[index_prev]);
				}
				else { // prev in & cur out
					pg_to.push_back(pg_from[index_prev]);

					if (pg_from[index_prev].pos[axis] != boundary) {
				...
				if (inside_cur && pg_from[index_cur].pos[axis] != boundary) { // prev out & cur in
					float t = fabsf((pg_from[index_prev].pos[axis] - boundary) / (pg_from[index_prev].pos[axis] - pg_from[index_cur].pos[axis]));
