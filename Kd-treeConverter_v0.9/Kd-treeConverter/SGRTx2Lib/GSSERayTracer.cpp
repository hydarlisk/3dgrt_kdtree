...
// work
// ------------------------------------------------------------------------------------------------
void GSSERayTracer::work( GThreadContext *pThreadContext ) 
{
	...
	switch (nPacketSize) {
		case 1:
			for (int i = nTID; i < m_iThreadJobSize; i+=m_iThreadCount) {
				m_SSERenderPipelineList[nTID]->Render1x1(i);
				//m_SSERenderPipelineList[nTID]->Split_Render1x1__PriRay(i);
			}
			break;
		case 2:
			for (int i = nTID; i < m_iThreadJobSize; i+=m_iThreadCount) {
				m_SSERenderPipelineList[nTID]->Render2x2(i);
				//m_SSERenderPipelineList[nTID]->Split_Render2x2__PriRay(i);
			}
			break;
		case 4:
			for (int i = nTID; i < m_iThreadJobSize; i+=m_iThreadCount) {
				m_SSERenderPipelineList[nTID]->Render4x4(i);
				//m_SSERenderPipelineList[nTID]->Split_Render4x4__PriRay(i);
			}
			break;
	}
...
GSSERayTracer::~GSSERayTracer(void)
{
	uninitialize();
}
...
// uninitialize()
// ------------------------------------------------------------------------------------------------
void GSSERayTracer::uninitialize()
{
#if GOThread
...
// rendering()
// ------------------------------------------------------------------------------------------------
GError GSSERayTracer::rendering( GScene *pScene, bool isDebug )
{
	GError error;
	...
	if (m_pScene->IsRun10Times() == true) {
		float fAvgTime = 0.0f;
		float fMaxTime = 0.0f, fMinTime = 10000.0f;
		...
		for(int i = 0; i < 10; i++) {
			...
			if(fMaxTime < timer1.getElapsedTime()) fMaxTime = timer1.getElapsedTime();
			if(fMinTime > timer1.getElapsedTime()) fMinTime = timer1.getElapsedTime();

			fAvgTime	+= timer1.getElapsedTime();
		}

		fAvgTime = (fAvgTime - fMinTime - fMaxTime)/8.0f;
...
// logging()
// ------------------------------------------------------------------------------------------------
void GSSERayTracer::logging_init()
{
	#if GOThread
		if ( m_iThreadCount > 0) {
			for ( int i = 0; i < m_iThreadCount; ++i ) {
...
void GSSERayTracer::logging_out()
{
