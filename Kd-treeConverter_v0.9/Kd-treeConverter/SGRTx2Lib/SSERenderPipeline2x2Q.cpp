...
void SSERenderPipeline::IsectPacket2x2( const KdTreeNode *node, int nIdx )
{
	...
	for (i = baseOffset; i < baseOffset+nObjs; i++) {
		...
		else { m_Mailbox[triID] = rp->RayId; }	// 무조건 isect 안되도 실행 되어야 함
...
// checkVisibility
// ---------------------------------------------------------------------------
void SSERenderPipeline::checkVisibility2x2(const GPoint* lightPos, const __m128 shadingmask) {
	...
	shadow_rp->d = sse_vsub(lPos, shadow_rp->o);
	shadow_rp->o = sse_vadd(shadow_rp->o, sse_vmul(shadow_rp->d, sse_vset1(EPSILON)));
...
void SSERenderPipeline::shading2x2 (int nIdx) {
	...
	for ( x = 3; x >= 0; x-- ) {
		if (rm->mask[i] && is->tacc[i] != 0) {
				...
				mat_fRough.f[x] = pMaterial->getRoughness();
				mat_fRefl.f[x] = pMaterial->getReflection();	if (mat_fRefl.f[x] > 0) b_refl = true;
				mat_fRefr.f[x] = pMaterial->getTransparency();	if (mat_fRefr.f[x] > 0) b_refr = true;
				mat_fRIdx.f[x] = pMaterial->getRefractionIndex();
				obj_num.f[x] = pObject->getObjectNumber();
	...
	_sse_vec oColor;
	_sse_vec N, R, L;
	union { __m128 shadingmask; __m128i ishadingmask; };
	union { __m128 islightmask; __m128i iislightmask; };
	union { __m128 visiblemask; __m128i ivisiblemask; };
		...
		if ( m_Scene->isEnableShadow() ) {
...

			{
				...
				oColor = sse_vupdate(sse_vadd(oColor, sse_vadd(
					...
					sse_vmul(sse_vmul(mat_cSpec, lColor), sse_vset1(sse_fast_pow(_mm_max_ps(_mm_setzero_ps(), sse_vdot(L, R)), mat_fRough.v4)))
						)), oColor, _mm_and_ps(visiblemask, _mm_andnot_ps(islightmask, shadingmask)));
		...
		} else {
			{
				...
				oColor = sse_vupdate(sse_vadd(oColor, sse_vadd(
					sse_vmul(sse_vmul(mat_cTex,  lColor), sse_vset1(_mm_max_ps(_mm_setzero_ps(), sse_vdot(L, N)))),
					sse_vmul(sse_vmul(mat_cSpec, lColor), sse_vset1(sse_fast_pow(_mm_max_ps(_mm_setzero_ps(), sse_vdot(L, R)), mat_fRough.v4)))
						)), oColor, _mm_andnot_ps(islightmask, shadingmask));
				// color += texColor * pLight->color * max( 0.0f, dot( L, N ) ) +
				//          specular * pLight->color * pow( max( 0.0f, dot( R, L ) ), roughness );
			}
		}
	...
	{
		if ( nIdx < (m_Scene->getMaxReflectionDepth())) {
			oColor = sse_vupdate(
	...
	if (nIdx < (m_Scene->getMaxReflectionDepth())) {
		...
		if (b_refl) {
			...
			_sse_2x2_raypacket	*refl_rp	= &m_RayPk2x2[nIdx+1];
			_sse_2x2_isect		*refl_is	= &m_Isect2x2[nIdx+1];
			_sse_2x2_raymask	*refl_rm	= &m_RMask2x2[nIdx+1];
			...
			InitPacket2x2( nIdx+1 );

			refl_rp->o.x4 = _mm_add_ps(hit_p.x4, _mm_mul_ps(refl_rp->d.x4, _mm_set1_ps(EPSILON)));
			refl_rp->o.y4 = _mm_add_ps(hit_p.y4, _mm_mul_ps(refl_rp->d.y4, _mm_set1_ps(EPSILON)));
			refl_rp->o.z4 = _mm_add_ps(hit_p.z4, _mm_mul_ps(refl_rp->d.z4, _mm_set1_ps(EPSILON)));

			unsigned int b;
			if (refl_rp->IsCoherent()) {
				refl_rp->RayWay = (refl_rp->xmask & 1) + (refl_rp->ymask & 2) + (refl_rp->zmask & 4);
				refl_rm->mask4 = _mm_and_ps( _mm_and_ps(refl_rm->mask4, rm->mask4), _mm_cmpgt_ps(mat_fRefl.v4, _mm_setzero_ps()));
				RenderPacket2x2( nIdx+1 );
				i = 3; for ( x = 3; x >= 0; x--, i-- ) {
					if (refl_is->tacc[i] && refl_rm->mask[i]) {
						is->color[i] += mat_fRefl.f[x] * refl_is->color[i] * GColor(mat_cDiff.x[x], mat_cDiff.y[x], mat_cDiff.z[x]);
					}
				}
			} else {
				// detect octants and render one by one
		...
		if (b_refr) {
			...
			_sse_2x2_raypacket	*refr_rp	= &m_RayPk2x2[nIdx+1];
			_sse_2x2_isect		*refr_is	= &m_Isect2x2[nIdx+1];
			_sse_2x2_raymask	*refr_rm	= &m_RMask2x2[nIdx+1];
			...
			refr_rp->o.x4 = _mm_add_ps(hit_p.x4, _mm_mul_ps(refr_rp->d.x4, _mm_set1_ps(EPSILON)));
			refr_rp->o.y4 = _mm_add_ps(hit_p.y4, _mm_mul_ps(refr_rp->d.y4, _mm_set1_ps(EPSILON)));
			refr_rp->o.z4 = _mm_add_ps(hit_p.z4, _mm_mul_ps(refr_rp->d.z4, _mm_set1_ps(EPSILON)));

			unsigned int b;
			if (refr_rp->IsCoherent()) {
				refr_rp->RayWay = (refr_rp->xmask & 1) + (refr_rp->ymask & 2) + (refr_rp->zmask & 4);
				refr_rm->mask4 = _mm_and_ps( _mm_and_ps(refr_rm->mask4, rm->mask4), _mm_cmpgt_ps(mat_fRefr.v4, _mm_setzero_ps()));
				RenderPacket2x2( nIdx+1 );
				i = 3; for ( x = 3; x >= 0; x--, i-- ) {
					if (refr_is->tacc[i] && refr_rm->mask[i]) {
						is->color[i] += mat_fRefr.f[x] * refr_is->color[i] * GColor(mat_cDiff.x[x], mat_cDiff.y[x], mat_cDiff.z[x]);
					}
				}
			} else {
				// detect octants and render one by one
...
void SSERenderPipeline::RenderPacket2x2( int nIdx )
{
	...
	//TravTime.end();
	//TravFreq += TravTime.getElapsedFreq();
	//TravCount += 1;
...
//		Tile renderer
// -----------------------------------------------------------
void SSERenderPipeline::Render2x2( int nThreadID )
{
	unsigned int xtiles = (m_Resolution.x) >> 1;
	unsigned int ytiles = (m_Resolution.y) >> 1;
	unsigned int tx, ty;
	...
	rp->o	= sse_vset1(m_Origin.x, m_Origin.y, m_Origin.z);
	delta4.d= sse_vset1(m_DX.x * 2, m_DX.y * 2, m_DX.z * 2);


	__m128 ox4 = _mm_load1_ps( &m_Origin.x );	//__m128 ox4 = broadcastss( &m_Origin.x );
	__m128 oy4 = _mm_load1_ps( &m_Origin.y );	//__m128 oy4 = broadcastss( &m_Origin.y );
	__m128 oz4 = _mm_load1_ps( &m_Origin.z );	//__m128 oz4 = broadcastss( &m_Origin.z );

	union { __m128i two4; unsigned int two[4]; };
	two[0] = two[1] = two[2] = two[3] = 2;

	int ytileSize = ytiles / m_MaxWorker;

	if (nThreadID == 100) {
		ty = 0;
		ytiles;
	} else if (nThreadID == (m_MaxWorker - 1)) {
		ty		= ytileSize * nThreadID;
		ytiles;
	} else {
		ty		= ytileSize * nThreadID;
		ytiles	= ytileSize * (nThreadID +1);
	}

	const int iCastSeq16_x[16] = { 0, 1, 0, 1, 2, 3, 2, 3, 0, 1, 0, 1, 2, 3, 2, 3 };
	const int iCastSeq16_y[16] = { 0, 0, 1, 1, 0, 0, 1, 1, 2, 2, 3, 3, 2, 2, 3, 3 };
	const float fSampSeq_x[5][16] = { { 0, }, { 0, }, { +0.25f, +0.75f, }, { 0.167f, 0.5f, 0.833f, }, { 0.125f, 0.375f, +0.625f, +0.875f, }};
	const float fSampSeq_y[5][16] = { { 0, }, { 0, }, { +0.25f, +0.75f, }, { 0.167f, 0.5f, 0.833f, }, { 0.125f, 0.375f, +0.625f, +0.875f, }};
	const float fSampWeight = 1.0f / (m_SuperSampling.x * m_SuperSampling.y);
	const float fXjitter = 0.5f / m_SuperSampling.x;	// jitterRate = 0.5f
	const float fYjitter = 0.5f / m_SuperSampling.y;	// jitterRate = 0.5f
	const float rcpSuperSampling = 1.0f / m_SuperSampling.x;
	...
	for ( ;ty < ytiles; ty++ ) {
		...
		// Set tpos
			...
			// tpos.d = m_LeftUp + m_DX * (float)iCastSeq16_x[i] - m_DY * (float)(ty * 2 + iCastSeq16_y[i]);
			tpos.d.x4 = _mm_add_ps(m_LeftUp4->x4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->x4, m_CastSeq2x2_x4[0]),
									  _mm_mul_ps(m_DY4->x4, _mm_add_ps(m_CastSeq2x2_y4[0],_mm_set1_ps(ty*2.0f)))));
			tpos.d.y4 = _mm_add_ps(m_LeftUp4->y4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->y4, m_CastSeq2x2_x4[0]),
									  _mm_mul_ps(m_DY4->y4, _mm_add_ps(m_CastSeq2x2_y4[0],_mm_set1_ps(ty*2.0f)))));
			tpos.d.z4 = _mm_add_ps(m_LeftUp4->z4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->z4, m_CastSeq2x2_x4[0]),
									  _mm_mul_ps(m_DY4->z4, _mm_add_ps(m_CastSeq2x2_y4[0],_mm_set1_ps(ty*2.0f)))));

			//is->addr[i] = iCastSeq16_x[i] + (m_Height - 1 - (ty * 2 + iCastSeq16_y[i])) * m_Width;
			is->addr4 = _mm_add_epi32(m_iCastSeq2x2_x4[0], _mm_sub_epi32(m_TmpAddr4[0], _mm_set1_epi32(m_Width*ty*2)));
...
//			for (int nSampY = 0; nSampY < m_SuperSampling.y; nSampY++) {
				...
				// Ray packet 을 셋팅 - 시작점(ox4, oy4, oz4) ~ 끝점(tpos)
...
//			}}	// Loops of (nSampleX * nSampleY)
			...
			is->addr4 = _mm_add_epi32( is->addr4, two4 );
		}
	}
}
