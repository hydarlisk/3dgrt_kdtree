...
// min
static __inline__ __host__ __device__ float2 min(float2 a, float2 b)
{
	return make_float2(min(a.x,b.x), min(a.y,b.y));
}

// max
static __inline__ __host__ __device__ float2 max(float2 a, float2 b)
{
...
// min
static __inline__ __host__ __device__ float3 min(float3 a, float3 b)
{
	return make_float3(min(a.x,b.x), min(a.y,b.y), min(a.z,b.z));
}

// max
static __inline__ __host__ __device__ float3 max(float3 a, float3 b)
{
...
{
    //float inv = 1.0f / s;
    return make_float3(s/(a.x), s/(a.y), s/(a.z));// a * inv;
}
inline __host__ __device__ void operator/=(float3 &a, float s)
{
    float inv = 1.0f / s;
