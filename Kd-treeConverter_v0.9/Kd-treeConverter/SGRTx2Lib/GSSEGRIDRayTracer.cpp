...
// work
// ------------------------------------------------------------------------------------------------
void GSSEGRIDRayTracer::work( GThreadContext *pThreadContext ) 
{
...
GSSEGRIDRayTracer::GSSEGRIDRayTracer()
{
	m_iOldSceneNumber = 0;
...
GSSEGRIDRayTracer::~GSSEGRIDRayTracer(void)
{
	uninitialize();
}
...
// uninitialize()
// ------------------------------------------------------------------------------------------------
void GSSEGRIDRayTracer::uninitialize()
{
#if GOThread
...
GError GSSEGRIDRayTracer::initialize( GScene *pScene )
{
	...
	if ( m_iOldSceneNumber != pScene->getSceneNumber() || 
		...
		error = pScene->getGridStructure()->makeSSERenderStructureInfo( m_pSSESceneData );		
		if ( error != errorNo )
			return error;
...
// rendering()
// ------------------------------------------------------------------------------------------------
GError GSSEGRIDRayTracer::rendering( GScene *pScene, bool isDebug )
{
	GError error;
	...
	if (m_pScene->getCPUPacketSize().x == 4) {
		...
		//grid rendering.
		m_pSSERenderPipeline->Grid_Rendering();
	}
	#endif
...
// logging()
// ------------------------------------------------------------------------------------------------
void GSSEGRIDRayTracer::logging_init()
{
	#if GOThread
		if ( m_iThreadCount > 0) {
			for ( int i = 0; i < m_iThreadCount; ++i ) {
...
void GSSEGRIDRayTracer::logging_out()
{
