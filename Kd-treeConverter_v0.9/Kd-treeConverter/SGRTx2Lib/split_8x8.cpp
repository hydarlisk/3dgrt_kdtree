...
// ------------------------------------------------------------------------------------------------
void SSERenderPipeline::Split_InitPkt8x8(int nIdx)
{
	int i;
	_sse_8x8_raypacket	*rp = &m_RayPk8x8[nIdx];
	_sse_8x8_isect		*is = &m_Isect8x8[nIdx];
	...
	rp->xmask =  _mm_movemask_ps( rp->d[0].x4 )       + (_mm_movemask_ps( rp->d[1].x4 ) << 4) +
				...
				(_mm_movemask_ps( rp->d[4].x4 ) << 16) + (_mm_movemask_ps( rp->d[5].x4 ) << 20);
				(_mm_movemask_ps( rp->d[6].x4 ) << 24) + (_mm_movemask_ps( rp->d[7].x4 ) << 28);
				(_mm_movemask_ps( rp->d[8].x4 ) << 32) + (_mm_movemask_ps( rp->d[9].x4 ) << 36);
				(_mm_movemask_ps( rp->d[10].x4 ) << 40) + (_mm_movemask_ps( rp->d[11].x4 ) << 44);
				(_mm_movemask_ps( rp->d[12].x4 ) << 48) + (_mm_movemask_ps( rp->d[13].x4 ) << 52);
				(_mm_movemask_ps( rp->d[14].x4 ) << 56) + (_mm_movemask_ps( rp->d[15].x4 ) << 60);
...
void SSERenderPipeline::Split_InitPkt8x8_ShwRay(void)
{
	int i;
	_sse_8x8_raypacket	*rp = &m_ShadowRayPk8x8[0];
	_sse_8x8_isect		*is = &m_ShadowIsect8x8[0];
...
bool SSERenderPipeline::Split_Isect8x8_PlaneTest_PriRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f, int temp )
{
	int j;
...
bool SSERenderPipeline::Split_Isect8x8_TriUVTest_PriRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f, RDATA4 &lambda, RDATA4 &mue, int temp )
{
	int j;
...
bool SSERenderPipeline::Split_Isect8x8_PlaneTest_SecRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f )
{
	int j;
...
bool SSERenderPipeline::Split_Isect8x8_TriUVTest_SecRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f, RDATA4 &lambda, RDATA4 &mue )
{
	int j;
...
bool SSERenderPipeline::Split_Isect8x8_PlaneTest_ShwRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f )
{
	int j;
...
bool SSERenderPipeline::Split_Isect8x8_TriUVTest_ShwRay( TriAccel2 &acc, int nIdx, RMASK4 &Mask_Hit, RDATA4 &f, RDATA4 &lambda, RDATA4 &mue )
{
	int j;
...
void SSERenderPipeline::Split_FCull_Init( __m128 &term1, __m128 &term2, const int baseOffset, int nIdx )
{
	...
	// Near / Far culling
	// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
	__m128 di = sse_inverse(frp->d.v4[I0]);
...
bool SSERenderPipeline::Split_FCull_TriEdge( int &check_aperture, const TriAccel2 &acc, const int negmaxabsdir )
{
	...
	den	= frp->d.v4[k] + _mm_set_ps1(acc.n_u) * frp->d.v4[ku] + _mm_set_ps1(acc.n_v) * frp->d.v4[kv];

	int densign = _mm_movemask_ps(den);
	if (densign == 0 || densign == 0xf ) {
		__m128 beam_tnom = _mm_set_ps1(acc.n_d) - 
				(frp->o.v4[k] + _mm_set_ps1(acc.n_u) * frp->o.v4[ku] + _mm_set_ps1(acc.n_v) * frp->o.v4[kv]);
		...
		if ( 1 ) {
			...
			hu		= frp->o.v4[ku] + t0 * frp->d.v4[ku];
			hv		= frp->o.v4[kv] + t0 * frp->d.v4[kv];
...
void SSERenderPipeline::Split_Isect8x8__PriRay( const KdTreeNode2 *node, int nIdx, int temp )
{
	int i, j;
...
PriIsect_FtnCall_Count++;
	...
	for (i = baseOffset+6; i < baseOffset+6+nObjs; i++) {
		...
		// -----------------------------------------------------------------------------
		RDATA4 f;
		if (!Split_Isect8x8_PlaneTest_PriRay(acc, nIdx, Mask_Hit, f, check_aperture)) continue;

		// -----------------------------------------------------------------------------
		// ==>> Triangle boundaries test (u, v)
		// -----------------------------------------------------------------------------
		RDATA4 lambda, mue;
		if (!Split_Isect8x8_TriUVTest_PriRay(acc, nIdx, Mask_Hit, f, lambda, mue, check_aperture)) continue;
...
void SSERenderPipeline::Split_Isect8x8__SecRay( const KdTreeNode2 *node, int nIdx )
{
	int i, j;
	...
	for (i = baseOffset; i < baseOffset+nObjs; i++) {
		...
		// -----------------------------------------------------------------------------
		RDATA4 f;
		if (!Split_Isect8x8_PlaneTest_SecRay(acc, nIdx, Mask_Hit, f)) continue;

		// -----------------------------------------------------------------------------
		// ==>> Triangle boundaries test (u, v)
		// -----------------------------------------------------------------------------
		RDATA4 lambda, mue;
		if (!Split_Isect8x8_TriUVTest_SecRay(acc, nIdx, Mask_Hit, f, lambda, mue)) continue;
...
void SSERenderPipeline::Split_Isect8x8__ShwRay( const KdTreeNode2 *node, int temp )
{
	int i, j;
	...
	for (i = baseOffset; i < baseOffset+nObjs; i++) {
		...
		// -----------------------------------------------------------------------------
		RDATA4 f;
		if (!Split_Isect8x8_PlaneTest_ShwRay(acc, temp, Mask_Hit, f)) continue;

		// -----------------------------------------------------------------------------
		// ==>> Triangle boundaries test (u, v)
		// -----------------------------------------------------------------------------
		RDATA4 lambda, mue;
		if (!Split_Isect8x8_TriUVTest_ShwRay(acc, temp, Mask_Hit, f, lambda, mue)) continue;
...
//		Sends a 8x8 packet through the tree
// -----------------------------------------------------------
void SSERenderPipeline::Split_Trace8x8__PriRay( int nIdx, int temp )
{
	int j;
	...
	while (1) {
		while (IS_LEAF(*node) == 0) {
			...
			__m128 d[4];
				...
				for (j = 3; j >= 0; j--) {
					m_Stack8x8[stackIndex].t_far_4[j] = t_far_4[j];
					m_Stack8x8[stackIndex].t_near4[j] = _mm_max_ps(t_near4[j], d[j]);
					t_far_4[j]		= _mm_min_ps(t_far_4[j],d[j]);
					Mask_Active4[j]	= _mm_and_ps(Mask_Active4[j], _mm_cmple_ps(t_near4[j], t_far_4[j]));
				}
				m_Stack8x8[stackIndex].node = BackSideSon;
				stackIndex++;
		}

		// Isect check
		Split_Isect8x8__PriRay(node, nIdx, temp);
		...
		// Stack pop
		--stackIndex;
		node			= m_Stack8x8[stackIndex].node;
		t_near4[0]		= m_Stack8x8[stackIndex].t_near4[0];
		t_near4[1]		= m_Stack8x8[stackIndex].t_near4[1];
		t_near4[2]		= m_Stack8x8[stackIndex].t_near4[2];
		t_near4[3]		= m_Stack8x8[stackIndex].t_near4[3];
		t_far_4[0]		= m_Stack8x8[stackIndex].t_far_4[0];
		t_far_4[1]		= m_Stack8x8[stackIndex].t_far_4[1];
		t_far_4[2]		= m_Stack8x8[stackIndex].t_far_4[2];
		t_far_4[3]		= m_Stack8x8[stackIndex].t_far_4[3];
...
void SSERenderPipeline::Split_Trace8x8__SecRay( int nIdx )
{
	int j;
	...
	while (1) {
		...
		// Isect check
		Split_Isect8x8__SecRay(node, nIdx);
...
void SSERenderPipeline::Split_Trace8x8__ShwRay( int temp )
{
	int j;
...
// Shading
// ---------------------------------------------------------------------------
void SSERenderPipeline::Split_Shading8x8 (const int nIdx) {
	...
	// Setup
	// *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
	Split_Shading8x8__Setup(nIdx, hit_p, global_ambt, mat_cAmbt, mat_cDiff, mat_cSpec, mat_cEmit,
	...
	if ( m_bIsEnableLocalShading ) {
		Split_Shading8x8__LocalShading(nIdx, hit_p, global_ambt, mat_cAmbt, mat_cDiff, mat_cSpec, mat_cEmit,
	...
	if (nIdx < m_iMaxReflectionDepth) {
		Split_Shading8x8_RayGeneration_SecRay(nIdx, hit_p,
		mat_fRefl, mat_fRefr, mat_fRIdx, mat_cTex, n_refl, n_refr);
	}
}

void SSERenderPipeline::Split_Shading8x8__Setup (const int nIdx, _sse_vec *hit_p,
	_sse_vec &global_ambt,
...
void SSERenderPipeline::Split_Shading8x8__LocalShading (const int nIdx, _sse_vec *hit_p,
	_sse_vec  &global_ambt,
	_sse_vec *mat_cAmbt, _sse_vec *mat_cDiff, _sse_vec *mat_cSpec, _sse_vec *mat_cEmit,
	_sse_float *mat_fRough,	_sse_float *mat_fRefl, _sse_float *mat_fRefr, _sse_float *mat_fRIdx,
	_sse_vec *mat_cTex,
	_sse_uint *obj_num)
{
	...
	_sse_8x8_raypacket	*shadow_rp;
	_sse_8x8_isect		*shadow_is;
	_sse_8x8_raymask	*shadow_rm;
	if ( m_bIsEnableShadow ) {
		shadow_rp	= &m_ShadowRayPk8x8[0];
		shadow_is	= &m_ShadowIsect8x8[0];
		shadow_rm	= &m_ShadowRMask8x8[0];
...
//		Generate & Trace Ray
// -----------------------------------------------------------
void SSERenderPipeline::Split_Shading8x8_RayGeneration_SecRay (const int nIdx, _sse_vec *hit_p,
	...
	_sse_vec *mat_cTex,	int n_refl, int n_refr)
{
	...
	if (rp->Depth < m_iMaxReflectionDepth) {
		...
		if (n_refl > 0) {
			...
			_sse_8x8_raypacket	*refl_rp	= &m_RayPk8x8[nNextIdx];
			_sse_8x8_isect		*refl_is	= &m_Isect8x8[nNextIdx];
			_sse_8x8_raymask	*refl_rm	= &m_RMask8x8[nNextIdx];
			...
			if (refl_rp->IsCoherent()) {
				...
				Split_Trace8x8__SecRay(nNextIdx);
				Split_Shading8x8(nNextIdx);
				i = 15; for ( y = 3; y >= 0; y-- ) for ( x = 3; x >= 0; x--, i-- ) {
		...
		if (n_refr > 0) {
			...
			_sse_8x8_raypacket	*refr_rp	= &m_RayPk8x8[nNextIdx];
			_sse_8x8_isect		*refr_is	= &m_Isect8x8[nNextIdx];
			_sse_8x8_raymask	*refr_rm	= &m_RMask8x8[nNextIdx];
			...
			if (refr_rp->IsCoherent()) {
				...
				refr_rm->mask4[0] = _mm_and_ps( _mm_and_ps(refr_rm->mask4[0], rm->mask4[0]), _mm_cmpgt_ps(mat_fRefr[0].v4, _mm_setzero_ps()));
				refr_rm->mask4[1] = _mm_and_ps( _mm_and_ps(refr_rm->mask4[1], rm->mask4[1]), _mm_cmpgt_ps(mat_fRefr[1].v4, _mm_setzero_ps()));
				refr_rm->mask4[2] = _mm_and_ps( _mm_and_ps(refr_rm->mask4[2], rm->mask4[2]), _mm_cmpgt_ps(mat_fRefr[2].v4, _mm_setzero_ps()));
				refr_rm->mask4[3] = _mm_and_ps( _mm_and_ps(refr_rm->mask4[3], rm->mask4[3]), _mm_cmpgt_ps(mat_fRefr[3].v4, _mm_setzero_ps()));
				Split_Trace8x8__SecRay(nNextIdx);
				Split_Shading8x8(nNextIdx);
				i = 15; for ( y = 3; y >= 0; y-- ) for ( x = 3; x >= 0; x--, i-- ) {
...
void SSERenderPipeline::Split_Shading8x8_RayGeneration_ShwRay(const _sse_vec objectPos[], const GPoint* lightPos, const __m128 shadingmask[]) {

	_sse_8x8_raypacket	*shadow_rp	= &m_ShadowRayPk8x8[0];
	_sse_8x8_isect		*shadow_is	= &m_ShadowIsect8x8[0];
	_sse_8x8_raymask	*shadow_rm	= &m_ShadowRMask8x8[0];
...
//		Tile renderer
// -----------------------------------------------------------
void SSERenderPipeline::Split_Render8x8__PriRay( int nThreadID )
{
	...
	// start spawning rays
	_sse_8x8_raypacket	*rp	= &m_RayPk8x8[0];
	_sse_8x8_isect		*is	= &m_Isect8x8[0];
	_sse_8x8_raymask	*rm	= &m_RMask8x8[0];
	_sse_8x8_raypacket	tpos;		// target position for ray casting (pixel center)
	_sse_8x8_raypacket	delta4;		// delta4 for next target position between packet8x8
	_sse_8x8_raypacket	jpos;		// jittered position for sampling
	...
	const int yTileSize = int(yTileEnd * m_fRcpThreadCount);
	...
	for ( ;ty < yTileEnd; ty++ ) {
		...
		for(i = 0; i < 4; i++) {
			...
			tpos.d[i].x4 = _mm_add_ps(m_LeftUp4->x4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->x4, m_CastSeq8x8_x4[i]),
									  _mm_mul_ps(m_DY4->x4, _mm_add_ps(_mm_set1_ps(ty<<2),m_CastSeq8x8_y4[i]))));
			tpos.d[i].y4 = _mm_add_ps(m_LeftUp4->y4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->y4, m_CastSeq8x8_x4[i]),
									  _mm_mul_ps(m_DY4->y4, _mm_add_ps(_mm_set1_ps(ty<<2),m_CastSeq8x8_y4[i]))));
			tpos.d[i].z4 = _mm_add_ps(m_LeftUp4->z4, _mm_sub_ps(
									  _mm_mul_ps(m_DX4->z4, m_CastSeq8x8_x4[i]),
									  _mm_mul_ps(m_DY4->z4, _mm_add_ps(_mm_set1_ps(ty<<2),m_CastSeq8x8_y4[i]))));

			// is->addr[i] = iCastSeq8x8_x[i] + (m_Height - 1 - (ty * 4 + iCastSeq8x8_y[i])) * m_Width;
			// m_Tmp2_Addr = (m_Height - 1 - iCastSeq8x8_y[i]) * m_Width
			is->addr4[i] = _mm_add_epi32(m_iCastSeq8x8_x4[i], _mm_sub_epi32(m_Tmp2_Addr4[i], _mm_set1_epi32((m_Width*ty)<<2)));
		}
		...
		for ( tx = 0; tx < xTileEnd; tx++ ) {
			...
			for (int nSampY = 0; nSampY < m_SuperSampling.y; nSampY++) {
				...
				rp->Depth = 0;
				Split_InitPkt8x8( 0 );	// direction vector normalize 등
//if (tx > 90)
				// -----------------------------------------------------------------------
